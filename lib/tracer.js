...
Tracer.prototype.extractTraceInfo = function extractTraceInfo(trace){
  ...
  var info = {
    ...
    child_duration: hrtToUs(trace.childExecTime || [0,0]),
...
Tracer.prototype.aggregate = function aggregate(traceInfo){
  if (this.newFrame){
    var self = this;
    setTimeout(function(){
  ...
  if (traceInfo.calledBy){
    ...
    source.child_duration += traceInfo.calledBy.child_duration;
  ...
  if( target.child_duration && target.duration){
    if( target.child_duration > target.duration){
      console.log("yikes, child longer than parent by  ", target.child_duration - target.duration, target.duration, target.id);
    }
  }
}