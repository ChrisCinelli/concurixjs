...
Tracer.prototype.funcAfterHook = function funcAfterHook(trace, globalState){
  ...
  if (this.running){
    ...
    if (nestCount > 1){
      ...
      //console.log("previous child time ", caller.childExecTime, caller.id);
      if( hrtToUs(caller.startTime) > hrtToUs(trace.startTime) ){
        console.log("yikes! child start before parent ", trace.id);
      }
...
Tracer.prototype.aggregate = function aggregate(traceInfo, rawTrace){
  ...
  if (traceInfo.calledBy){
    ...
    source.child_duration += hrtToUs(rawTrace.childExecTime); // traceInfo.calledBy.child_duration;
  ...
  //TODO
  if( target.child_duration && target.duration){
    if( target.child_duration > target.duration){
      console.log(target.nest_level, "yikes, child longer than parent by  ", target.child_duration - target.duration, target.duration, target.id);
    }
  }
}