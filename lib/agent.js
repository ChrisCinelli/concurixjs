...
Agent.prototype.startTracer = function startTracer(){
  ...
  this._tracer.on('trace', function(trace){
    self.onTrace(trace);
  });
  this._tracer.start();
}
...
Agent.prototype.stopTracer = function stopTracer(){
  ...
  if (_tracer){
    _tracer.removeListener('trace', this.onTrace);
    _tracer.restoreRequire();
    this._tracer = null;
  }
}

Agent.prototype.onTrace = function onTrace(trace){
  if (!this.socket){ return; }
  trace.trace_name = this.config.traceName;
  //TODO: do we need UTF8 encoding before sending over the socket?
  var str = JSON.stringify({ cxCmd: 'trace', trace: trace }) + '\0';
