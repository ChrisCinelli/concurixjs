...
Functions.parse = function parse(fname) {
    if (!parsed[fname]) {
        fs.readFile(fname, function(err, data){
            if (!err) {
                log('Parse file ' + fname);
                var ast = esprima.parse(data.toString(), parser_options);
                traverse_ast(ast, fname);
                log(Object.keys(table).length + ' functions in table');
            } else {
                log('Error parsing ' + fname);
            }
        });
        parsed[fname] = true;
    }
    ...
    if (node.fun_name == 'anonymous') {
        ...
        if (filename) {
            ...
            if (descriptions && descriptions.length == 1) {
                ...
                log('no info for ' + filename + ':' + node.line);
            }
        }
    }
    return node;
...
function traverse_ast(ast, fname) {
...
function context_summary(context) {
    switch (context.kind) {
    case esprima.Syntax.AssignmentExpression:
