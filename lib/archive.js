...
function acquireAwsCredentials(userCallback) {
  ...
  var host        = 'api.concurix.com';       // Change to localhost for local testing
  var port        =                 80;       // Change to        80 for local testing

  var custKey     = process.env.accountKey;
  var machineName = require('os').hostname();

  var options     = {
                      host:   host,
                      port:   port,
                      path:   '/v1/' + custKey + '/new_run/' + machineName,
                      method: 'POST'
                    };
