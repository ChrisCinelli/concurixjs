...
function createOrUpdateLink(src, trg, type, trace){
  ...
  if( lnk.type == 'callback'){
    delay = util.hrtToUs(trace.startTime)
    //console.log("trace.start ", trace, "callback ", trace.callbackOf);
    lnk.total_delay += trace.start - trace.callbackOf.start;
    if( trace.start < trace.callbackOf.start){
      console.log("yikes, target starting before source! by ", trace.start - trace.callbackOf.start);
    }
  } else {
    lnk.total_delay += trace.start - trace.calledBy.start;
    if( trace.start < trace.calledBy.start){
      console.log("yikes, target starting before source! by ", trace.start - trace.calledBy.start);
    }
